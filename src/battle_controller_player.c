...
static u8 TypeEffectiveness(u8 targetId, u32 battler, u16 move, u16 type)
{
    // todo account for hidden power and maybe other dynamic move types?
    uq4_12_t modifier;

    modifier = CalcTypeEffectivenessMultiplier(move, type, battler, targetId, GetBattlerAbility(targetId), TRUE);
...
//ravetodo account for ivy cudgel (see 1.7.0 expansion update)
//ravetodo revisit this for font sizing and idk maybe other shit
static void MoveSelectionDisplayMoveTypeDoubles(u8 targetId, u32 battler)
{
	u8 *txtPtr;
	struct ChooseMoveStruct *moveInfo = (struct ChooseMoveStruct*)(&gBattleResources->bufferA[battler][4]);
    ...
    u8 typeColor = TypeEffectiveness(targetId, battler, moveType, move);
...
static void MoveSelectionDisplayMoveType(u32 battler)
{
    ...
    {
        typeColor = TypeEffectiveness(GetBattlerAtPosition(BATTLE_OPPOSITE(GetBattlerPosition(battler))), battler, move, moveType);
    }
    ...
    if (typeColor != COLOR_EFFECTIVE) {
        *(end)++ = EXT_CTRL_CODE_BEGIN;
        *(end)++ = EXT_CTRL_CODE_FONT;
        *(end)++ = FONT_NORMAL;

        switch (typeColor) {
            case COLOR_IMMUNE:
                StringCopy(end, gText_MoveInterfaceImmune);
                break;
            case COLOR_NOT_VERY_EFFECTIVE:
                StringCopy(end, gText_MoveInterfaceNotVeryEffective);
                break;
            case COLOR_SUPER_EFFECTIVE:
                StringCopy(end, gText_MoveInterfaceSuperEffective);
                break;
        }
    }

    PrependFontIdToFit(txtPtr, end, FONT_NORMAL, WindowWidthPx(typeColor) - 25);
    BattlePutTextOnWindow(gDisplayedStringBattle, typeColor);
...
static void MoveSelectionDisplayInfo(u32 battler)
{
    ...
    u32 power = CalcMoveBasePower(move, battlerAtk, battlerDef, defAbility, weather);
    u32 accuracy = gMovesInfo[move].accuracy;
