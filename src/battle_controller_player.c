...
static void MoveSelectionDisplayMoveTypeDoubles(u8 targetId, u32 battler)
{
	...
	struct ChooseMoveStruct *moveInfo = (struct ChooseMoveStruct*)(&gBattleResources->bufferA[battler][4]);
    u8 moveType = gBattleMoves[moveInfo->moves[gMoveSelectionCursor[battler]]].type;
...
void PlayerHandleExpUpdate(u32 battler)
{
    ...
    if (GetMonData(&gPlayerParty[monId], MON_DATA_LEVEL) >= MAX_LEVEL || LevelCapped(GetMonData(&gPlayerParty[monId], MON_DATA_LEVEL)))
    {
        PlayerBufferExecCompleted(battler);
    }
    else
...
static void MoveSelectionDisplayInfo(u32 battler)
{
    ...
    u32 accuracy = gBattleMoves[move].accuracy;
